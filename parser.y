...
%start cmd_line
%token <string> ALIAS APPEND BYE CD END ERR IN OUT STRING UNALIAS PRINTENV SETENV UNSETENV PIPING
...
cmd_line    :
    ...
    | SETENV STRING STRING STRING END {setenv($2, $3, $4); return 1;}
    | UNSETENV STRING END           {unsetenv($2); return 1;}
    | PRINTENV END                  {printenv(); return 1;}
    | PIPING END                    {piping(); return 1;}
...
int setenv(const char * var, const char * word, int overwrite){
	for (int i = 0; i < varIndex; i++) {
		...
		else if((strcmp(varTable.var[i], var) == 0) && (overwrite == 0)){
			return 1;
		}
		else if((strcmp(varTable.var[i], var) == 0) && (overwrite != 0)){
